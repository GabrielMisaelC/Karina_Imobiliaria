...
if (isset($_POST["btn_cadastro"])) {
    ...
    $sql = "INSERT INTO `TB_INQ`(`NOME_INQ`, `CPF_INQ`, `CELULAR`, `CONDICAO_PAGAMANTO_INQ`, `REN_INQ`, `PROFISSAO_INQ`, `NACIONALIDADE_INQ`, `ENDERECO_ATUAL_INQ`, `ESTADO_CIVIL_INQ`, `RG_INQ`, `DATA_NASCIMENTO_INQ`) VALUES ('$nome','$cpf','$celular','$condicao_pag','$rne','$profissao','$nacionalidade','$endereco_atual','$estado_civil','$rg','$data_nascimento')";
     ...
     $id_inq = mysqli_insert_id($db);
    
			/*
			if($r){
			  echo"<script language='javascript' type='text/javascript'>alert('Inquilino cadastrado com sucesso!');window.location.href='inicio.php?link=2'</script>";
			}else{
			  echo"<script language='javascript' type='text/javascript'>alert('Não foi possível cadastrar esse Inquilino');window.location.href='inicio.php?link=2'</script>";
			}
            */

    $id_imovel = $_POST["id_imv"];
    $id_pro = $_POST["cod_Pro"];
    $data_inicio = $_POST["data_inicio"];
    $prazo = $_POST["prazo"];
    $valor = "222";

    $sql = "INSERT INTO `TB_CONTRATO`(`ID_IMV`, `ID_PRO`, `ID_INQ`, `VALOR_MENSAL`, `PRAZO_CONT`, `DATA_INICIO_CONT`) VALUES ('$id_imovel','$id_pro','$id_inq','$valor','$prazo','$data_inicio')";

    $r = mysqli_query($db, $sql) or die("Não foi possivel inserir dados");

    if($r){
        echo"<script language='javascript' type='text/javascript'>alert('Contrato cadastrado com sucesso!');window.location.href='inicio.php?link=2'</script>";
      }else{
        echo"<script language='javascript' type='text/javascript'>alert('Não foi possível cadastrar esse Contrato');window.location.href='inicio.php?link=2'</script>";
      }
